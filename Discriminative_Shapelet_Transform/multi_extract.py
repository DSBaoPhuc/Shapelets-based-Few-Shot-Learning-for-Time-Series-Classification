...
def load_ts_file(file_path):
    ...
    X = np.array(X, dtype=np.float32)  # shape: (n_samples, n_dimensions, series_length)
...
def z_norm_fast(ts):
    ...
    std = np.std(ts)
    if std < 1e-8:
        out = np.empty(ts.shape, dtype=np.float32)
...
def subdist_fast(x, y):
    x_norm = z_norm_fast(x)
    y_norm = z_norm_fast(y)
    s = 0.0
    for i in range(x_norm.shape[0]):
        diff = x_norm[i] - y_norm[i]
        s += diff * diff
    return np.sqrt(s)


@njit(fastmath=True)
def compute_distances_batch(shapelet, X):
    ...
    for i in range(n_samples):
        ...
        for start in range(0, series_len - m + 1):
            sub = series[start:start + m]
            d = subdist_fast(shapelet, sub)
            if d < best:
                best = d
        distances[i] = best
    return distances
...
def separability_score(distances, labels):
    ...
    d1 = distances[labels == unique[0]]
    d2 = distances[labels == unique[1]]
    mean_diff = abs(np.mean(d1) - np.mean(d2))
    pooled_std = np.sqrt(0.5 * (np.var(d1) + np.var(d2))) + 1e-8
    return float(mean_diff / pooled_std)
...
def evaluate_shapelets(shapelets, X, y, weights=(0.4, 0.3, 0.3)):
    w_ig, w_f, w_sep = weights
    results = []

    for i, shapelet in enumerate(tqdm(shapelets, desc="Evaluating shapelets", leave=False)):
        shapelet = np.array(shapelet, dtype=np.float32)
        distances = compute_distances_batch(shapelet, X)
        ...
        results.append((i, len(shapelet), ig, f_stat, sep, comp))
...
def generate_candidate_shapelets(X, min_len=10, max_len=50, step=5, max_per_series=None):
    """
    Sinh các shapelet ứng viên (các subsequence từ từng chuỗi)
    """
    candidates = []
    n_samples, series_len = X.shape
    for i in range(n_samples):
        ...
        for L in range(min_len, min(max_len, series_len) + 1, step):
            ...
            for start in range(0, series_len - L + 1, stride):
                sl = ts[start:start + L].astype(np.float32).copy()
                candidates.append(sl)
                if max_per_series is not None and len(candidates) >= max_per_series:
                    return candidates
    return candidates

def save_topk_shapelets(df, shapelets, k=10, out_file="top_shapelets.csv"):
    """
    Lưu top-k shapelet (và chỉ số đánh giá) ra file CSV.
    Ngoài các cột thống kê, mỗi shapelet cũng được lưu chuỗi giá trị dạng text.
    """
    top_df = df.head(k).copy()
    top_shapelets = [shapelets[idx] for idx in top_df['id']]
    top_df['Shapelet_Values'] = [",".join(map(lambda x: f"{x:.5f}", s)) for s in top_shapelets]

    top_df.to_csv(out_file, index=False)
    print(f"Saved top-{k} shapelets to: {out_file}")


# ===========================
# Main: load data + extract + evaluate
# ===========================
...
if __name__ == "__main__":
    ...
    print(f"\nExtracting candidate shapelets from dimension {dim} ...")
    candidates = generate_candidate_shapelets(X_dim, min_len=10, max_len=40, step=10, max_per_series=500)
    print(f"Total candidates: {len(candidates)}")

    print("\nEvaluating shapelets ...")
    df_results = evaluate_shapelets(candidates, X_dim, y)
    print(df_results.head(10))

    print(f"\nDone in {time.time() - start_time:.2f} seconds.")

    save_topk_shapelets(df_results, candidates, k=10, out_file="top_shapelets.csv")
